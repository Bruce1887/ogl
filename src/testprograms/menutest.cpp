...
void initializeWorld(Scene** scene, TerrainGenerator** terrainGen, TerrainChunkManager** chunkManager, 
                     game::Player** player, game::GameClock** gameClock, Skybox** skybox, 
...
int main(int, char **)
{
    ...
    game::GameClock* gameClock = nullptr;
    Skybox* skybox = nullptr;
    ThirdPersonCamera* camController = nullptr;
    
    // Setup menu skybox
    std::vector<std::string> menuSkyboxFaces = {
    ...
    Shader* menuSkyboxShader = new Shader();
    menuSkyboxShader->addShader("Skybox.vert", ShaderType::VERTEX);
    menuSkyboxShader->addShader("Skybox.frag", ShaderType::FRAGMENT);
    menuSkyboxShader->createProgram();
    menuSkyboxShader->bind();
    menuSkyboxShader->setUniform("skybox", 0);
    
    uiManager.setMenuSkybox(menuSkybox, menuSkyboxShader);
    ...
    // Cleanup
    if (scene) cleanupWorld(scene, terrainGen, chunkManager, player, gameClock, skybox, camController);
    delete menuSkybox;
    delete menuSkyboxShader;
                     ...
                     ThirdPersonCamera** camController)
{
    ...
    *terrainGen = new TerrainGenerator();
    ...
    int vertexStep = 5;
    int gc_threshold = 50;
    *chunkManager = new TerrainChunkManager(*terrainGen, chunkSize, vertexStep, terrainTextures, gc_threshold);
    (*chunkManager)->setShader(terrainShader);

    // Player
    *player = new game::Player(glm::vec3(100, 0, 100), (MODELS_DIR / "cow" / "cow.obj").string());

    // Game clock
    *gameClock = new game::GameClock();
