...
int main(int, char **)
{
    if (chdir("/mnt/g/Plugg/pcs/ogl") != 0) {
        // If chdir fails (permissions, path doesn't exist), print error.
        std::cerr << "WARNING: Could not change directory to project root." << std::endl;
    }
        ...
        g_InputManager->movementInput.updateMovement(key, action, mods); });
    {
        ...
        GameClock* testClock = new GameClock(120.0f, 12.0f); // 120s day, starts at noon

        // Set initial test values to show bar movement
        testPlayer->setHealth(95.0f); // Use the getter from Entity.h
        testPlayer->setStamina(70.0f); // Use the setter from Player.h
        
        // Ensure OpenGL Blending is enabled for transparent HUD elements
        GLCALL(glEnable(GL_BLEND));
        GLCALL(glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA));

        // Create the HUD object
        HUD* testHUD = new HUD(testPlayer, testClock, window_X, window_Y);

        // create the scene
        Scene scene(camera, lightSource);

        // Assign HUD to scene if supported; use scene.setHUD(testHUD) if available.
        // Otherwise keep HUD separate to avoid accessing a non-existent member.
        // scene.setHUD(testHUD); // uncomment if implemented
        ...
        while (!glfwWindowShouldClose(g_window))
        {
            ...
            // 1. Advance the Clock
            testClock->Update(deltaTime);
            
            // 2. Test Player Vitals (Health Decay)
            if (testPlayer->getHealth() > 5.0f) {
                // Decrease health to show the red bar shrink
                testPlayer->HandleHit(deltaTime * 1.5f); 
            }
            
            // 3. Test Stamina Drain/Regen (Use spacebar input)
            float currentStamina = testPlayer->getStamina();
            
            if (glfwGetKey(g_window, GLFW_KEY_SPACE) == GLFW_PRESS) {
                // Sprinting: drain stamina fast
                currentStamina -= deltaTime * 30.0f;
            } else {
                // Idle: regenerate stamina slowly
                currentStamina += deltaTime * 15.0f;
            }
            
            // Clamp stamina to [0, 100] (Assuming max is 100.0f)
            currentStamina = glm::clamp(currentStamina, 0.0f, 100.0f); 
            // NOTE: setStamina is missing in your Player.h; we use the private member access for quick test
            // We'll rely on the public setter if you added it, otherwise you need a public setter.
            // For now, let's assume you added: void setStamina(float s) { stamina = s; } 
            testPlayer->setStamina(currentStamina);
