      ...
      m_screenHeight(screenHeight)
{
    try {
        ...
        if (!m_shader) {
            std::cerr << "Failed to create pause menu shader" << std::endl;
            return;
        }
        ...
        std::cerr << "Exception creating pause menu shader: " << e.what() << std::endl;
        m_shader = nullptr;
    }
    ...
    try {
        ...
        std::cout << "PauseMenu text renderer initialized successfully" << std::endl;
    } catch (const std::exception& e) {
...
void PauseMenu::handleMouseClick(double mouseX, double mouseY, bool pressed)
{
    for (auto& button : m_buttons)
    {
        ...
        if (contains)
        {
            ...
            {
                std::cout << "Pause menu button clicked: " << button.label << std::endl;
                button.onClick();
            }
        }
        else
