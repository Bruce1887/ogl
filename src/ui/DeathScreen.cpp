      ...
      m_screenHeight(screenHeight)
{
    ...
    try
    {
        ...
        m_textRenderer->LoadFont((FONTS_DIR / "DejaVuSans.ttf").string(), MENU_FONT_SIZE);
    }
    catch (const std::exception& e)
    {
    ...
    // Setup dimensions using scaled UIConfig helpers
    float sw = (float)screenWidth;
    float sh = (float)screenHeight;

    // Setup text input box dimensions - scaled
    m_inputBoxWidth = UIConfig::inputBoxWidth(sw, sh);
    m_inputBoxHeight = UIConfig::inputBoxHeight(sw, sh);
    m_inputBoxX = UIConfig::centerX(m_inputBoxWidth, sw);
    m_inputBoxY = sh * UIConfig::INPUT_BOX_Y_RATIO;

    // Setup submit button dimensions - scaled
    m_submitButtonWidth = UIConfig::submitButtonWidth(sw, sh);
    m_submitButtonHeight = UIConfig::submitButtonHeight(sw, sh);
    m_submitButtonX = UIConfig::centerX(m_submitButtonWidth, sw);
    m_submitButtonY = sh * UIConfig::SUBMIT_Y_RATIO;

    // Setup continue/skip button dimensions - scaled
    m_buttonWidth = UIConfig::submitButtonWidth(sw, sh);
    m_buttonHeight = UIConfig::buttonHeight(sw, sh) * 0.67f;
    m_buttonX = UIConfig::centerX(m_buttonWidth, sw);
    m_buttonY = sh * UIConfig::CONTINUE_Y_RATIO;
}
...
void DeathScreen::render(glm::mat4 view, glm::mat4 projection, PhongLightConfig* phongLight)
{
    ...
    // Player name text with cursor - scaled
    ...
    DrawTextLeft(displayText,
             m_inputBoxX + 10.0f * scale,
             m_inputBoxY + m_inputBoxHeight * 0.5f + 8.0f * scale,
             0.8f * scale,
             glm::vec4(1.0f, 1.0f, 1.0f, m_fadeAlpha));
    ...
    // Submit button text - scaled
    DrawText("Submit Score",
             m_submitButtonX + m_submitButtonWidth * 0.5f,
             m_submitButtonY + m_submitButtonHeight * 0.5f + 8.0f * scale,
             0.9f * scale,
             glm::vec4(1.0f, 1.0f, 1.0f, m_fadeAlpha));
    ...
    // Continue button text - scaled
    DrawText("Skip",
             m_buttonX + m_buttonWidth * 0.5f,
             m_buttonY + m_buttonHeight * 0.5f + 6.0f * scale,
             0.7f * scale,
...
void DeathScreen::updateScreenSize(int width, int height)
{
    ...
    // Recalculate all dimensions with new screen size
    m_inputBoxWidth = UIConfig::inputBoxWidth(sw, sh);
    m_inputBoxHeight = UIConfig::inputBoxHeight(sw, sh);
    m_inputBoxX = UIConfig::centerX(m_inputBoxWidth, sw);
    m_inputBoxY = sh * UIConfig::INPUT_BOX_Y_RATIO;
